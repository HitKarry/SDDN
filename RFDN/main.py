...
class Trainer:
    def __init__(self, configs):
        ...
        self.network = RFDN(configs).to(configs.device)
...
if __name__ == '__main__':
    ...
    model = RFDN(configs).to(configs.device)
    net = torch.load('checkpoint_0.04881389.chk')
    ...
    np.savez('result04881389.npz', sr=pred.cpu()*data_std[None, :, None, None]+data_mean[None, :, None, None], hr=label.cpu()*data_std[None, :, None, None]+data_mean[None, :, None, None])