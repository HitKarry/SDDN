...
class Trainer:
    def __init__(self, configs):
        ...
        self.network = EDSR(configs).to(configs.device)
    ...
    def test(self, dataloader_test):
        ...
        with torch.no_grad():
            for i,(lr,hr) in enumerate(dataloader_test):
                sr = self.network(lr.float().to(self.device))
                sr_inter = F.interpolate(lr.float().to(self.device), scale_factor=self.configs.scale[0], mode='bilinear', align_corners=False)
...
if __name__ == '__main__':
    ...
    model = EDSR(configs).to(configs.device)
    net = torch.load('checkpoint_0.048456427.chk')
    ...
    np.savez('result048456427.npz', ilr=ILR*data_std[None, :, None, None]+data_mean[None, :, None, None], sr=pred.cpu()*data_std[None, :, None, None]+data_mean[None, :, None, None], hr=label.cpu()*data_std[None, :, None, None]+data_mean[None, :, None, None])
