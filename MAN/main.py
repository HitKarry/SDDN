...
class Trainer:
    def __init__(self, configs):
        ...
        torch.manual_seed(0)
        self.network = MAN(configs).to(configs.device)
...
if __name__ == '__main__':
    ...
    model = MAN(configs).to(configs.device)
    net = torch.load('checkpoint_0.050485477.chk')
    ...
    # ILR = F.interpolate(torch.tensor(data_lr), scale_factor=configs.scale[0], mode='bilinear', align_corners=False)

    np.savez('result050485477.npz', sr=pred.cpu()*data_std[None, :, None, None]+data_mean[None, :, None, None], hr=label.cpu()*data_std[None, :, None, None]+data_mean[None, :, None, None])