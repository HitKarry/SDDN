...
class Trainer:
    def __init__(self, configs):
        ...
        self.network = DDBPN(configs).to(configs.device)
...
if __name__ == '__main__':
    ...
    model = DDBPN(configs).to(configs.device)
    net = torch.load('checkpoint_0.04908962.chk')
    ...
    np.savez('result04908962.npz', ilr=ILR*data_std[None, :, None, None]+data_mean[None, :, None, None], sr=pred.cpu()*data_std[None, :, None, None]+data_mean[None, :, None, None], hr=label.cpu()*data_std[None, :, None, None]+data_mean[None, :, None, None])